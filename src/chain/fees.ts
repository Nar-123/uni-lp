...
import { poolAbi, stateViewAbi } from './abis.js';
...
/** Pure fee calc when pool globals + tick outsides are already loaded (batch /list path). */
export function computeV3UnclaimedFeesFromData(params: {
  tickLower: number;
  tickUpper: number;
  liquidity: bigint;
...
export async function computeV3UnclaimedFees(params: {
  ...
  try {
    ...
    return computeV3UnclaimedFeesFromData({
      tickLower,
      tickUpper,
      liquidity,
      feeGrowthInside0LastX128,
      ...
      tokensOwed1,
      currentTick,
      feeGrowthGlobal0X128: g0 as bigint,
      feeGrowthGlobal1X128: g1 as bigint,
      feeGrowthOutside0LowerX128: tickL[2] as bigint,
      feeGrowthOutside1LowerX128: tickL[3] as bigint,
      feeGrowthOutside0UpperX128: tickU[2] as bigint,
      feeGrowthOutside1UpperX128: tickU[3] as bigint,
    });
  } catch (e) {
